...
module Decidim
  module EnhancedTextwork
    module Admin
      describe ParagraphsSplitForm do
        subject { form }

        let(:paragraphs) { create_list(:paragraph, 2, component: component) }
        ...
        context "when merging to the same component" do
          ...
          context "when a paragraph has an endorsement" do
            before do
              create(:endorsement, resource: paragraphs.sample, author: build(:user, organization: component.participatory_space.organization))
            end
