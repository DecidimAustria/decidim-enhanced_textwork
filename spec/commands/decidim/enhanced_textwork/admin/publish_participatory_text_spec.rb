    ...
    module Admin
      describe PublishParticipatoryText do
        describe "call" do
          ...
          let(:paragraphs) do
            paragraphs = create_list(:paragraph, 3, :draft, component: current_component)
          ...
          it "creates a version for each paragraph", versioning: true do
            expect { command.call }.to broadcast(:ok)

            paragraphs.each do |paragraph|
              expect(paragraph.reload.versions.count).to eq(1)
            end
          end
