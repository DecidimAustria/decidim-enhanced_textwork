...
shared_examples "a paragraph form" do |options|
  subject { form }
  ...
  context "when geocoding is enabled" do
    let(:component) { create(:paragraph_component, :with_geocoding_enabled, participatory_space: participatory_space) }
    ...
    context "when latitude and longitude are manually set" do
      ...
      context "when the paragraph is unchanged" do
        let(:previous_paragraph) { create(:paragraph, address: address) }

        let(:title) do
          if options[:skip_etiquette_validation]
            previous_paragraph.title
          else
            translated(previous_paragraph.title)
          end
        end

        let(:body) do
          if options[:skip_etiquette_validation]
            previous_paragraph.body
          else
            translated(previous_paragraph.body)
          end
        end

        let(:params) do
          {
            id: previous_paragraph.id,
            title: title,
            body: body,
            author: previous_paragraph.authors.first,
            category_id: previous_paragraph.try(:category_id),
            scope_id: previous_paragraph.try(:scope_id),
            has_address: has_address,
            address: address,
            attachment: previous_paragraph.try(:attachment_params),
            latitude: latitude,
            longitude: longitude
          }
        end
  ...
  it "properly maps category id from model" do
    paragraph = create(:paragraph, component: component, category: category)

    expect(described_class.from_model(paragraph).category_id).to eq(category_id)
  end

  if options && options[:user_group_check]
    it "properly maps user group id from model" do
      paragraph = create(:paragraph, component: component, users: [author], user_groups: [user_group])

      expect(described_class.from_model(paragraph).user_group_id).to eq(user_group_id)
    end
  end
...
shared_examples "a paragraph form with meeting as author" do |options|
  subject { form }
