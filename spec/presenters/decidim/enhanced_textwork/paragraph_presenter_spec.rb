...
module Decidim
  module EnhancedTextwork
    describe ParagraphPresenter, type: :helper do
      subject(:presenter) { described_class.new(paragraph) }

      let(:paragraph) { build(:paragraph, body: content) }
      ...
      describe "#versions", versioning: true do
        ...
        it { is_expected.to eq(paragraph.versions) }

        context "when paragraph has an answer that wasn't published yet" do
          before do
            paragraph.update!(answer: "an answer", state: "accepted", answered_at: Time.current)
          end
        ...
        context "when a paragraph's answer gets published" do
          let(:paragraph) { create(:paragraph) }
