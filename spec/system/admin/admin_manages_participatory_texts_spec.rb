...
describe "Admin manages participatory texts", type: :system do
  ...
  def import_document
    ...
    expect(page).to have_content "The following sections have been converted to paragraphs. Now you can review and adjust them before publishing."
    expect(page).to have_content "Preview participatory text"
    validate_paragraphs
  end
  ...
  def publish_participatory_text
    ...
    expect(page).to have_content "All paragraphs have been published"
  end

  def validate_published
    paragraphs = Decidim::EnhancedTextwork::Paragraph.where(component: current_component)
    titles = [
    ...
    expect(paragraphs.count).to eq(titles.size)
    expect(paragraphs.published.count).to eq(titles.size)
    expect(paragraphs.published.order(:position).pluck(:title).map(&:values).map(&:first)).to eq(titles)
  end
  ...
  def edit_participatory_text_body(index, new_body)
    fill_in(
      "preview_participatory_text_paragraphs_attributes_#{index}_body",
      with: new_body
    )
  end
  ...
  describe "accessing participatory texts in draft mode" do
    let!(:paragraph) { create :paragraph, :draft, component: current_component, participatory_text_level: "section" }

    it "renders only draft paragraphs" do
      visit_participatory_texts
  ...
  describe "discarding participatory texts in draft mode" do
    let!(:paragraphs) { create_list(:paragraph, 5, :draft, component: current_component, participatory_text_level: "article") }

    it "removes all paragraphs in draft mode" do
      visit_participatory_texts
  ...
  describe "updating participatory texts in draft mode" do
    let!(:paragraph) { create :paragraph, :draft, component: current_component, participatory_text_level: "article" }
    let!(:new_body) { Faker::Lorem.unique.sentences(number: 3).join("\n") }

    it "persists changes and all paragraphs remain as drafts" do
      visit_participatory_texts
      ...
      validate_occurrences(sections: 0, subsections: 0, articles: 1)
      paragraph.reload
      expect(translated(paragraph.body).delete("\r")).to eq(new_body)
    end
  end
