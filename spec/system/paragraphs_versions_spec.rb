...
describe "Explore versions", versioning: true, type: :system do
  ...
  let!(:paragraph) { create(:paragraph, body: { en: "One liner body" }, component: component, skip_injection: true) }
  let!(:emendation) { create(:paragraph, body: { en: "Amended One liner body" }, component: component, skip_injection: true) }
  let!(:amendment) { create :amendment, amendable: paragraph, emendation: emendation }

  let(:form) do
    Decidim::Amendable::ReviewForm.from_params(
      id: amendment.id,
      amendable_gid: paragraph.to_sgid.to_s,
  ...
  context "when visiting a paragraph details" do
    before do
  ...
  context "when visiting versions index" do
    ...
    it "allows going back to the paragraph" do
      click_link "Go back to paragraph"
      expect(page).to have_current_path paragraph_path
    end
  ...
  context "when showing version" do
    ...
    it "allows going back to the versions list" do
      ...
      expect(page).to have_current_path "#{paragraph_path}/versions"
    end
    ...
    it "shows the changed attributes" do
      ...
      within ".diff-for-body" do
        ...
        within ".diff > ul > .del" do
          expect(page).to have_content(translated(paragraph.body))
        end
