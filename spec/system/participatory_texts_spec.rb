...
describe "Participatory texts", type: :system do
  ...
  def should_have_paragraph(selector, paragraph)
    expect(page).to have_tag(selector, text: translated(paragraph.title))
    prop_block = page.find(selector)
    prop_block.hover
    clean_paragraph_body = strip_tags(translated(paragraph.body))

    expect(prop_block).to have_button("Follow")
    expect(prop_block).to have_link("Comment") if component.settings.comments_enabled
    expect(prop_block).to have_link(paragraph.comments_count.to_s) if component.settings.comments_enabled
    expect(prop_block).to have_content(clean_paragraph_body) if paragraph.participatory_text_level == "article"
    expect(prop_block).not_to have_content(clean_paragraph_body) if paragraph.participatory_text_level != "article"
  end

  shared_examples_for "lists all the paragraphs ordered" do
    it "by position" do
      visit_component
      expect(page).to have_css(".hover-section", count: paragraphs.count)
      paragraphs.each_with_index do |paragraph, index|
        should_have_paragraph("#paragraphs div.hover-section:nth-child(#{index + 1})", paragraph)
      end
    end

    context "when participatory text level is not article" do
      it "not renders the participatory text body" do
        paragraph_section = paragraphs.first
        paragraph_section.participatory_text_level = "section"
        paragraph_section.save!
        visit_component
        should_have_paragraph("#paragraphs div.hover-section:first-child", paragraph_section)
      end
    end

    context "when participatory text level is article" do
      it "renders the paragraph body" do
        paragraph_article = paragraphs.last
        paragraph_article.participatory_text_level = "article"
        paragraph_article.save!
        visit_component
        should_have_paragraph("#paragraphs div.hover-section:last-child", paragraph_article)
      end
    end
  end
  ...
  shared_examples "showing the Amend button and amendments counter when hovered" do
    ...
    it "shows the Amend button and amendments counter inside the paragraph div" do
      visit_component
      paragraph_title = translated(paragraphs.first.title)
      find("#paragraphs div.hover-section", text: paragraph_title).hover
      within all("#paragraphs div.hover-section").first, visible: :visible do
  ...
  shared_examples "hiding the Amend button and amendments counter when hovered" do
    it "hides the Amend button and amendments counter inside the paragraph div" do
      visit_component
      paragraph_title = translated(paragraphs.first.title)
      find("#paragraphs div.hover-section", text: paragraph_title).hover
      within all("#paragraphs div.hover-section").first, visible: :visible do
  ...
  context "when listing paragraphs in a participatory process as participatory texts" do
    ...
    context "when admin has published a participatory text" do
      ...
      let!(:paragraphs) { create_list(:paragraph, 3, :published, component: component) }
      let!(:component) do
        create(:paragraph_component,
               :with_participatory_texts_enabled,
               manifest: manifest,
      ...
      it_behaves_like "lists all the paragraphs ordered"
      ...
      context "with existing amendments" do
        let!(:emendation_1) { create(:paragraph, :published, component: component) }
        let!(:amendment_1) { create :amendment, amendable: paragraphs.first, emendation: emendation_1 }
        let!(:emendation_2) { create(:paragraph, component: component) }
        let!(:amendment_2) { create(:amendment, amendable: paragraphs.first, emendation: emendation_2) }
