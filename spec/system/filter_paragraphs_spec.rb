...
describe "Filter Paragraphs", :slow, type: :system do
  ...
  context "when filtering paragraphs by ORIGIN" do
    context "when official_paragraphs setting is enabled" do
      before do
        component.update!(settings: { official_paragraphs_enabled: true })
      end
      ...
      context "with 'official' origin" do
        it "lists the filtered paragraphs" do
          create_list(:paragraph, 2, :official, component: component, scope: scope)
          create(:paragraph, component: component, scope: scope)
          visit_component
          ...
          expect(page).to have_css(".card--paragraph", count: 2)
          expect(page).to have_content("2 PARAGRAPHS")
        end
      end

      context "with 'citizens' origin" do
        it "lists the filtered paragraphs" do
          create_list(:paragraph, 2, component: component, scope: scope)
          create(:paragraph, :official, component: component, scope: scope)
          visit_component
    ...
    context "when official_paragraphs setting is not enabled" do
      before do
        component.update!(settings: { official_paragraphs_enabled: false })
      end
  ...
  context "when filtering paragraphs by SCOPE" do
    ...
    before do
      create_list(:paragraph, 2, component: component, scope: scope)
      create(:paragraph, component: component, scope: scope2)
      create(:paragraph, component: component, scope: nil)
      visit_component
    end
    ...
    context "when selecting the global scope" do
      it "lists the filtered paragraphs", :slow do
        within ".filters .scope_id_check_boxes_tree_filter" do
          uncheck "All"
          check "Global"
        end

        expect(page).to have_css(".card--paragraph", count: 1)
        expect(page).to have_content("1 PARAGRAPH")
      end
    end
    ...
    context "when selecting the global scope and another scope" do
      it "lists the filtered paragraphs", :slow do
        ...
        expect(page).to have_content("3 PARAGRAPHS")
      end
    end
  ...
  context "when filtering paragraphs by STATE" do
    context "when paragraph_answering component setting is enabled" do
      before do
        component.update!(settings: { paragraph_answering_enabled: true })
      end

      context "when paragraph_answering step setting is enabled" do
        before do
          component.update!(
            step_settings: {
        ...
        it "lists accepted paragraphs" do
          create(:paragraph, :accepted, component: component, scope: scope)
          visit_component
          ...
          within ".card--paragraph" do
            expect(page).to have_content("ACCEPTED")
          end
        end

        it "lists the filtered paragraphs" do
          create(:paragraph, :rejected, component: component, scope: scope)
          visit_component
        ...
        context "when there are paragraphs with answers not published" do
          let!(:paragraph) { create(:paragraph, :accepted_not_published, component: component, scope: scope) }
          ...
          it "shows only accepted paragraphs with published answers" do
            within ".filters .state_check_boxes_tree_filter" do
              check "All"
              uncheck "All"
              check "Accepted"
          ...
          it "shows accepted paragraphs with not published answers as not answered" do
            within ".filters .state_check_boxes_tree_filter" do
              check "All"
              uncheck "All"
              check "Not answered"
      ...
      context "when paragraph_answering step setting is disabled" do
        before do
          component.update!(
            step_settings: {
              component.participatory_space.active_step.id => {
    ...
    context "when paragraph_answering component setting is not enabled" do
      before do
        component.update!(settings: { paragraph_answering_enabled: false })
      end
  ...
  context "when filtering paragraphs by CATEGORY", :slow do
    context "when the user is logged in" do
      ...
      let!(:paragraph1) { create(:paragraph, component: component, category: category) }
      let!(:paragraph2) { create(:paragraph, component: component, category: category2) }
      let!(:paragraph3) { create(:paragraph, component: component, category: category3) }
  ...
  context "when filtering paragraphs by ACTIVITY" do
    let(:active_step_id) { component.participatory_space.active_step.id }
    let!(:voted_paragraph) { create(:paragraph, component: component) }
    let!(:vote) { create(:paragraph_vote, paragraph: voted_paragraph, author: user) }
    let!(:paragraph_list) { create_list(:paragraph, 3, component: component) }
    let!(:created_paragraph) { create(:paragraph, component: component, users: [user]) }
    ...
    context "when the user is logged in" do
      ...
      it "can be filtered by my paragraphs" do
        within "form.new_filter" do
          expect(page).to have_content(/My paragraphs/i)
        end
      end

      it "lists the filtered paragraphs created by the user" do
        within "form.new_filter" do
          find("input[value='my_paragraphs']").click
        end
        expect(page).to have_css(".card--paragraph", count: 1)
      end
      ...
      context "when votes are enabled" do
        ...
        it "lists the filtered paragraphs voted by the user" do
          within "form.new_filter" do
            find("input[value='voted']").click
          end

          expect(page).to have_css(".card--paragraph", text: translated(voted_paragraph.title))
        end
      end
  ...
  context "when filtering paragraphs by TYPE" do
    context "when there are amendments to paragraphs" do
      let!(:paragraph) { create(:paragraph, component: component, scope: scope) }
      let!(:emendation) { create(:paragraph, component: component, scope: scope) }
      ...
      context "with 'all' type" do
        it "lists the filtered paragraphs" do
          ...
          expect(page).to have_css(".card.card--paragraph", count: 2)
          expect(page).to have_content("2 PARAGRAPHS")
          expect(page).to have_content("Amendment", count: 2)
        end
      end

      context "with 'paragraphs' type" do
        it "lists the filtered paragraphs" do
          within ".filters" do
            choose "Paragraphs"
          end

          expect(page).to have_css(".card.card--paragraph", count: 1)
          expect(page).to have_content("1 PARAGRAPH")
      ...
      context "when amendments_enabled component setting is enabled" do
        ...
        context "and amendments_visbility component step_setting is set to 'participants'" do
          ...
          context "when the user is logged in" do
            context "and has amended a paragraph" do
              let!(:new_emendation) { create(:paragraph, component: component, scope: scope) }
              let!(:new_amendment) { create(:amendment, amendable: paragraph, emendation: new_emendation, amender: new_emendation.creator_author) }
            ...
            context "and has NOT amended a paragraph" do
              before do
                login_as user, scope: :user
                visit_component
              end
          ...
          context "when the user is logged in" do
            context "and has amended a paragraph" do
              ...
              it "lists all the amendments" do
                ...
                expect(page).to have_css(".card.card--paragraph", count: 2)
                expect(page).to have_content("2 PARAGRAPH")
  ...
  context "when using the browser history", :slow do
    before do
      ...
      create_list(:paragraph, 2, :official, component: component)
      create_list(:paragraph, 2, :official, :accepted, component: component)
      create_list(:paragraph, 2, :official, :rejected, component: component)
    ...
    it "recover filters from initial pages" do
      ...
      expect(page).to have_css(".card.card--paragraph", count: 8)

      page.go_back

      expect(page).to have_css(".card.card--paragraph", count: 6)
    end
